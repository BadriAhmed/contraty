...
export default function V2GeneratePage() {
  ...
  const handleGenerateWithAd = () => {
    ...
    trackEvent("contract_generate", { slug: type, lang, field_count: totalFields });
    handleGenerate(false, totalFields + 1, setCurrentStep);
  };

  const handleAdComplete = () => setShowAdLoading(false);

  const handleDownloadWithTracking = (format: "pdf" | "docx") => {
    trackEvent("contract_download", { slug: type, lang, format });
